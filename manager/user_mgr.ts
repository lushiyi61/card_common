 ...
 * 数据存储区域
 */
const user_map_account: Map<number, string> = new Map(); // K:user_id V:account
const user_map_socket: Map<number, any> = new Map(); // K:user_id V:socket
 ...
 * 绑定玩家和socket关系
 */
function bind_socket(user_id, socket) {
...
function send_user_msg(user_id, event, msgdata) {
...
function get_user_table_id(user_id) {
    return user_map_table[user_id];
}

async function get_user_info_async(user_id) {
